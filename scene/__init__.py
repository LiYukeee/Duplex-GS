...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], ply_path=None, logger=None, viewer=False):
        """
        ...
        if not viewer:  # viewer no need to load train/test dataset images
            self.train_cameras = {}
            self.test_cameras = {}
            ...
            for resolution_scale in self.resolution_scales:
                ...
                # camera_center = torch.zeros([len(self.train_cameras[resolution_scale]), 3],
                #                             device=self.train_cameras[resolution_scale][0].camera_center.device)
                # for idx in range(len(self.train_cameras[resolution_scale])):
                #     camera_center[idx] = self.train_cameras[resolution_scale][idx].camera_center
                # self.scene_center = torch.mean(camera_center, dim=0)
                # # self.scene_radii = 1.1 * torch.max(torch.norm(camera_center - self.scene_center, dim=1))
                # self.scene_radii = torch.max(torch.norm(camera_center - self.scene_center, dim=1))
